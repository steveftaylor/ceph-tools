...
# Copyright (C) 2018-2023 Stephen Taylor
#
...
osd_disk_size = {int(fields[0]): float(fields[1]) * 1024 for fields in [x.split() for x in command_output("""ceph --cluster {} osd df -f json | jq -r '.nodes[] | "\(.id) \(.kb)"'""".format(cluster)).split('\n') if x != '']}
...
crush_weight = {int(fields[0]): float(fields[1]) for fields in [x.split() for x in command_output("""ceph --cluster {} osd df -f json | jq -r '.nodes[] | "\(.id) \(.crush_weight)"'""".format(cluster)).split('\n') if x != '']}
...
def get_osd_variance():
    ...
    pg_data = [x for x in command_output("""ceph --cluster {} pg dump pgs -f json 2> /dev/null | jq -r '.pg_stats[] | "\(.pgid) \(.stat_sum.num_bytes) \(.up)"'""".format(cluster)).split('\n') if x != '']
    pgs = []
    pg_size = {}
    pg_osds = {}
...
while num_reweight_attempts < max_reweight_attempts:
    osd = max({osd:abs(1 - variance) for osd, variance in osd_variance.items()}.items(), key=operator.itemgetter(1))[0]
